...
const Detail = () => {
  ...
  const changeStatus = async (id,status)=>{
        try{
        const response =await axios.put(`https://employee-backend-nu.vercel.app/api/leave/${id}`,status,{
          headers:{
            Authorization: `Bearer ${localStorage.getItem('token')}`
          }
        })
                 ...
                 <div className='flex space-x-3 mb-2'>
                    ...
                    {leave.status === "Pending" ? (
                        <div className='flex space-x-2'>
                            <button className='px-2 py-0.5 bg-teal-300 hover:bg-teal-400 rounded-full'
                                onClick={()=> changeStatus(leave._id, {status:"Approved"})}
                            >Approve</button>
                            <button className='px-2 py-0.5 bg-red-300 hover:bg-red-400 rounded-full'
